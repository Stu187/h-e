<?php
/**
 * Almost all of the below code belongs to 'Get the Image' WordPress plugin by Justin Tadlock. This is the perfect code for extracting post images, I have just changed the function * names specific to Featured Slider, as someone would like to use the 'Get the Image' WordPress plugin separately. 
 ...
 * is found, call featured_sslider_display_the_image() to format it.
 *
 * @since 0.1
 ...
 */
function featured_sslider_get_the_image( $args = array() ) {
	global $post;
	...
	$args = apply_filters( 'featured_sslider_get_the_image_args', $args );
	...
	extract( $args );
		...
		if ( $custom_key )
			$image = featured_sslider_image_by_custom_field( $args );
 ...
 * @return array|bool
 */
function featured_sslider_image_by_custom_field( $args = array() ) {
 ...
 * other filters are later added in the featured_sslider_display_the_image() function.
 *
 * @since 0.4
 * @param array $args
 * @return array|bool
 */
function featured_sslider_image_by_the_post_thumbnail( $args = array() ) {
	...
	$alt = trim( strip_tags( get_post_field( 'post_title', $post_thumbnail_id ) ) );
 ...
 * @return array|bool
 */
function featured_sslider_image_by_attachment( $args = array() ) {
	...
	if ( empty( $attachments ) && $args['size'] ) {
		if ( 'attachment' == get_post_type( $args['post_id'] ) ) {
			...
			$alt = trim( strip_tags( get_post_field( 'post_title', $args['post_id'] ) ) );
		}
	}
	...
	foreach ( $attachments as $id => $attachment ) {
		if ( ++$i == $args['order_of_image'] ) {
			...
			$alt = trim( strip_tags( get_post_field( 'post_title', $id ) ) );
			break;
		}
	}
...
/**
 * Scans the post for images within the content.  Not called by default with featured_sslider_get_the_image().
 ...
 * @return array|bool
 */
function featured_sslider_image_by_scan( $args = array() ) {
 ...
 * given in an array.  Not used with featured_sslider_get_the_image() by default.
 *
 * @since 0.3
 * @param array $args
 * @return array
 */
function featured_sslider_image_by_default( $args = array() ) {
 ...
 */
function featured_sslider_display_the_image( $args = array(), $image = false ) {
	...
	$data_tran = ( ( $data_tran ) ?   ' '.'data-anim="'.$data_tran.'"'  : '' );
	...
	if($cropping=='1'){
	  ...
	  $html = '<img src="'.bfi_thumb( $image['url'], $params ).'" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" '.$data_tran.'" class="' . esc_attr( $class ) .'" ' . $width . $height .' '.$style.' />';	  
    }
	else{
	  $html = '<img src="' . $image['url'] . '" alt="' . esc_attr( strip_tags( $image_alt ) ) . '" '.$data_tran.' class="' . esc_attr( $class ) . '" ' . $width . $height .' '.$style.'  />';
	}
	
	/* If $link_to_post is set to true, link the image to its post. */
	if ( $link_to_post ) {
		$image_title=($image_title_text=='1')?( 'title="' . esc_attr( apply_filters( 'the_title', get_post_field( 'post_title', $post_id ) ) ) . '" '):'';
		$html = '<a href="' . get_permalink( $post_id ) . '" '.$image_title.' '.$a_attr.'>' . $html . '</a>';}
	else {
	    if($permalink!='') {
	    
	    		if($args['a_attr_lbox']=='swipe-box') {
				 $photobox='swipebox featured_thumb_anchor';
			}
			elseif($args['a_attr_lbox']=='smooth-box') {
			...
			$image_title=($image_title_text=='1')?( 'title="' . esc_attr( apply_filters( 'the_title', get_post_field( 'post_title', $post_id ) ) ) . '" '):'';
			$html = '<a class="'.$photobox.'" href="' . $permalink . '" '.$image_title.' '.$a_attr.'>' . $html . '</a>';
		}
	}
...
/**
 * Get the image with a link to the post.  Use featured_sslider_get_the_image() instead.
 *
 * @since 0.1
 * @deprecated 0.3
 */
function featured_sslider_get_the_image_link( $deprecated = '', $deprecated_2 = '', $deprecated_3 = '' ) {
	featured_sslider_get_the_image();
}
